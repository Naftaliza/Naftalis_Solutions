        ...
        >
          ...
          >
            <div className="flex items-center justify-between p-6 border-b border-slate-200">
              ...
              <Button onClick={() => setIsCartOpen(false)} variant="ghost" size="icon" className="text-slate-500 hover:bg-slate-100">
                <X />
              </Button>
            </div>
            <div className="flex-grow p-6 overflow-y-auto space-y-4 bg-slate-50">
              {cartItems.length === 0 ? (
                ...
                cartItems.map(item => (
                  <div key={item.variant.id} className="flex items-center gap-4 bg-white p-3 rounded-lg border border-slate-200">
                    <img src={item.product.image} alt={item.product.title} className="w-20 h-20 object-cover rounded-md border border-slate-200" />
                    ...
                    <div className="flex flex-col items-end gap-2">
                      <div className="flex items-center border border-slate-300 rounded-md">
                        <Button onClick={() => updateQuantity(item.variant.id, Math.max(1, item.quantity - 1))} size="sm" variant="ghost" className="px-2 text-slate-600 hover:bg-slate-200">-</Button>
                        <span className="px-2 text-slate-800">{item.quantity}</span>
                        <Button onClick={() => updateQuantity(item.variant.id, item.quantity + 1)} size="sm" variant="ghost" className="px-2 text-slate-600 hover:bg-slate-200">+</Button>
                      </div>
            ...
            {cartItems.length > 0 && (
              <div className="p-6 border-t border-slate-200 bg-white">
                ...
                <Button onClick={handleCheckout} className="w-full bg-teal-500 hover:bg-teal-600 text-white font-semibold py-3 text-base">
                  Proceed to Checkout
                </Button>
              </div>
            )}
